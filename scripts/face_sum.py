...
if __name__=='__main__':
	...
	# face sum ON means
	z_mean_man_w_glasses = torch.mean(z_men_w_glasses[:1], dim=0, keepdim=True)
	z_mean_man_wout_glasses = torch.mean(z_men_wout_glasses[:1], dim=0, keepdim=True)
	z_mean_woman_wout_glasses = torch.mean(z_women_wout_glasses[:1], dim=0, keepdim=True)
