...
if __name__=='__main__':
	...
	try:
		...
		z_men_w_glasses = torch.Tensor(z_men_w_glasses).cuda()[[6,8,13,16,18]]#[[0,2,6,17,25,29]]
		z_men_wout_glasses = np.load(join(exDir, 'z_'+opts.labels[0]+'_wout_'+opts.labels[1]+'.npy'))
		z_men_wout_glasses = torch.Tensor(z_men_wout_glasses).cuda()[[1,3,7,12,14,15]]#[[1,3,5]]#[[3,4,5,9,12,13,16,20,22,23,26,32,33]]
		z_women_wout_glasses = np.load(join(exDir, 'z_not_'+opts.labels[0]+'_wout_'+opts.labels[1]+'.npy'))
		z_women_wout_glasses = torch.Tensor(z_women_wout_glasses).cuda()[[2,3,4,6,7,8,9,13,15,18]]
